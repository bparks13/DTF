function [AR,C]=estimate_ar_coefficients(X,m,method)
%% [AR,C]=estimate_ar_coefficients(X,m,method)
%
...
%    - method: String defining whether to use Yule-Walker equations ['yule', default] or
%       to use the arfit methods from the Signal Processing for Neuroscientists book
%       ['arfit'] 
%
%   Outputs:
...
disp('WARNING: THIS METHOD IS BUGGY AND LIKELY INCORRECT')
...
%             [R_all(:,i),lags] = xcov(X(:,a),X(:,b),m,'coeff');
        end
...
%                 C=C+reshape(R_all(lags==-i,:),numSeries,numSeries)' * -AR(:,:,i); 
                C = C + AR(:,:,i) * reshape(R_all(x,:),numSeries,numSeries); 
            end
        end
    end
elseif strcmp(method,'arfit')
    [~,AR_tmp,C,~,~,~]=arfit(X,m,m);
    
    AR=zeros(numSeries,numSeries,m);
    
    for i=1:m
        AR(:,:,i)=AR_tmp(:,(i-1)*numSeries+1:i*numSeries);
    end
end

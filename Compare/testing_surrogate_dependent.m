...
sig{1}=calculate_significance_from_surrogate(surrogate_filt,alpha(1),'dependent');
sig{2}=calculate_significance_from_surrogate(surrogate_filt,alpha(2),'dependent');
...
sig{3}=calculate_significance_from_surrogate(tmp_surr1,alpha(3),'dependent');
sig{4}=calculate_significance_from_surrogate(tmp_surr1,alpha(4),'dependent');
...
sig{5}=calculate_significance_from_surrogate(tmp_surr2,alpha(5),'dependent');
sig{6}=calculate_significance_from_surrogate(tmp_surr2,alpha(6),'dependent');
    ...
    for k=1:numChannels
        for l=1:numChannels
            if k ~= l
                ...
                for i=1:6
                    plot(freqForAnalysis,squeeze(sig{i}.(fields{j})(k,l,:)),'Color',colors(i,:),'LineWidth',2); 
                    hold on;

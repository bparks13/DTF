...
%% Directed Transfer Function: Unified Asymptotic Theory and Some of Its Implications, 3 Var AR(2)
...
N=1000;
m=2;
C_orig=[1,5,0.3;5,100,2;0.3,2,1]; %stdZ=sqrt(C_orig); 
a=[0.95*sqrt(2),0,0.35;0.5,0.5,0;0,1,-0.5]; a=cat(3,a,[-0.9025,0,0;0,0,0;0,0,0]);

X1=create_data(N,m,C_orig,a); % Give Covariance matrix instead of stdZ values because correlated
...
% N=500;
% X2=create_data(N,m,C_orig,a);
% [mdl_mse2,E_mse2,crit_mse2]=mvar(X2,config);
% 
% subplot(224); plot(orderRange,crit_mse2); title('MSE (10,000 Points)'); 

%% Directed Transfer Function: Unified Asymptotic Theory and Some of Its Implications, 3 variable AR(2) model - Figure 2

mdl_orig=struct('AR',a,'order',2,'numSeries',3);
[conn_orig,H_orig]=dtf(mdl_orig,spectral_range,fs);
...
config_plot=struct('seriesType',7);
plot_connectivity(conn_orig,S_orig,spectral_range,{'Sig1','Sig2','Sig3'},config_plot);

hFig=gcf;
hFig.Position=[hFig.Position(1:2) 1000 700];

%% Directed Transfer Function: Unified Asymptotic Theory and Some of Its Implications, 3 variable AR(2) model - Figure 3
...
config_plot=struct('seriesType',7);
plot_connectivity(conn_est,S_est,spectral_range,{'Sig1','Sig2','Sig3'},config_plot);

hFig=gcf;
hFig.Position=[hFig.Position(1:2) 1000 700];

%% Directed Transfer Function: Unified Asymptotic Theory and Some of Its Implications, 5 variable AR(2) model - Figure 1
...
%% Directed Transfer Function: Unified Asymptotic Theory and Some of Its Implications, 5 variable AR(2) model - Figure 2

mdl_orig=struct('AR',a,'order',2,'numSeries',5);
[conn_orig,H_orig]=dtf(mdl_orig,spectral_range,fs);

S_orig=calculate_spectra(H_orig,C_orig);

config_plot=struct('seriesType',7);
plot_connectivity(conn_orig,S_orig,spectral_range,{'Sig1','Sig2','Sig3','Sig4','Sig5'},config_plot);
hFig=gcf;
hFig.Position=[round(hFig.Position(1:2)./2) 1338 1026];

%% Directed Transfer Function: Unified Asymptotic Theory and Some of Its Implications, 5 variable AR(2) model - Figure 3

% Using bic for modeling

[conn_bic,H_bic]=dtf(mdl_bic,spectral_range,fs);

S_bic=calculate_spectra(H_bic,mdl_bic.C);

config_plot=struct('seriesType',7);
plot_connectivity(conn_bic,S_bic,spectral_range,{'Sig1','Sig2','Sig3','Sig4','Sig5'},config_plot);
hFig=gcf;
hFig.Position=[round(hFig.Position(1:2)./2) 1338 1026];
